 ...
 * @param filePath Full path to file to read install info from
 * @private
 */
...
module.exports = function (pkg) {
  ...
  return client.packages.byName({package: pkg})
    .then(function (data) {
    ...
    .then(function (data) {
      ...
      return Q.nfcall(downloader, {source: data.repo, dest: data.dest})
        .then(function () {
          return data;
        });
    })
    .then(function (data) {
      return Q.all([_buildInstaller('npm install', path.join(data.dest, 'package.json')), _buildInstaller('bower install', path.join(data.dest, 'bower.json'))])
        .then(function () {
          return data.dest;
        });
    });
