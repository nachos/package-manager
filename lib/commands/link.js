...
var _recreate = function (src, dest) {
  return Q.nfcall(rimraf, dest).then(function () {
    // Should we uninstall instead? return uninstall(data.name).then(function () {
    return Q.nfcall(fs.symlink, src, dest, 'junction').then(function () {
...
module.exports = function (src) {
  ...
  return Q.nfcall(jf.readFile, path.join(src, 'nachos.json')).then(function (data) {
    return packages.getFolderByType(data.type).then(function (folderPath) {
      ...
      return Q.nfcall(fs.lstat, data.dest).then(function (stats) {
        if (stats && stats.isSymbolicLink()) {
          return Q.nfcall(fs.readlink, data.dest).then(function (link) {
